...
- `FR/tables/04_pruefung_verifikation/short_results_table.tex`: kompakte LaTeX-Ergebnistabelle für den Haupttext.
- `FR/tables/04_pruefung_verifikation/full_results_table.tex`: ausführliche LaTeX-Tabelle für Anhang oder Querformatseite.
- `FR/tables/04_pruefung_verifikation/data_quality_table.tex`: Tabelle zur Datengrundlage und Bereinigung.
- `FR/tables/04_pruefung_verifikation/short_results_table.svg`: gerenderte LaTeX-Ergebnistabelle als SVG.
- `FR/tables/04_pruefung_verifikation/full_results_table.svg`: gerenderte ausfuehrliche LaTeX-Tabelle als SVG.
- `FR/tables/04_pruefung_verifikation/data_quality_table.svg`: gerenderte Tabelle zur Datengrundlage als SVG.
- `FR/tables/04_pruefung_verifikation/tables.md`: Markdown-Kopie der Ergebnistabellen.

## Diagramme

- `FR/images/04_pruefung_verifikation/chart_paired_differences.svg`: Paarweise Differenzen. Zeigt für jedes Paar die Differenz Workflow minus Direkter Aufruf.
- `FR/images/04_pruefung_verifikation/chart_paired_differences_sample.svg`: Paarweise Differenzen (Sample). Zeigt dieselbe Darstellung fuer die ersten 120 Paare. Diese Version ist besser lesbar, aber nicht vollstaendig.
- `FR/images/04_pruefung_verifikation/chart_paired_differences_part_001.svg usw.`: Paarweise Differenzen in Teilen. Zusaetzliche mehrteilige Version mit bis zu 100 Paaren pro SVG. Diese Dateien sind besser lesbar als der vollstaendige Chart und decken zusammen alle Paare ab.
- `FR/images/04_pruefung_verifikation/chart_difference_distribution.svg`: Verteilung der Paardifferenzen. Zeigt, ob der Workflow-Effekt stabil ist oder stark streut.
- `FR/images/04_pruefung_verifikation/chart_scoreverteilung_direkter_aufruf.svg`: Verteilung der Scores des direkten Aufrufs. Zeigt, wie viele Antworten des direkten Aufrufs bereits nahe oder genau bei 100% lagen. Das macht den Deckeneffekt sichtbar.
- `FR/images/04_pruefung_verifikation/chart_direkter_aufruf_vs_workflow_scatter.svg`: Scorevergleich Direkter Aufruf vs Workflow. Punkte oberhalb der Diagonale bedeuten, dass der Workflow höher bewertet wurde als der direkte Aufruf. Gleiche Score-Kombinationen werden leicht versetzt und mit ihrer Anzahl beschriftet.
