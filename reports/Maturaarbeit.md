...
Diese Arbeit untersucht die Frage, ob KI-Workflows qualitativ bessere Antworten liefern als einzelne KI-Aufrufe. Unter einem KI-Workflow werden mehrere aufeinanderfolgende Verarbeitungsschritte verstanden, bei denen KI die Resultate weiter überprüft, verbessert oder zusätzliche Informationen beschafft. Da solche Workflows zunehmend in Softwarelösungen eingesetzt werden, stellt sich die Frage, ob sie tatsächlich einen messbaren Nutzen bringen und die Qualität von KI-generierten Antworten erhöhen.

Zur Durchführung der Untersuchung wurde eine eigene Software entwickelt, mit der verschiedene KI-Workflows erstellt, automatisiert ausgeführt, ausgewertet und statistisch verglichen werden konnten. Die Anwendung ermöglichte die Durchführung umfangreicher Testreihen, die automatische Erfassung der Resultate sowie die Erstellung von Diagrammen und statistischen Auswertungen. Dadurch konnte eine grosse Anzahl von Antwortpaaren systematisch untersucht werden.

Verglichen wurden verschiedene Workflow-Typen mit direkten Einzelaufrufen. Die Qualität der Antworten wurde anhand mehrerer Kriterien beurteilt, darunter Richtigkeit, Vollständigkeit, Relevanz, Verständlichkeit, Quellenqualität, Verifikation, Umgang mit Unsicherheit sowie Rückfragefähigkeit. Für jede Kategorie wurden eigene Fragesets und Bewertungskriterien erstellt. Die statistische Auswertung erfolgte mithilfe eines zweiseitigen gepaarten t-Tests. Zusätzlich wurde ein Kontrollversuch mit computer-auswertbaren mathematischen Aufgaben durchgeführt, um mögliche Verzerrungen durch KI-basierte Bewertungen zu reduzieren und die Ergebnisse zu überprüfen.

Die Resultate zeigen, dass die untersuchten KI-Workflows insgesamt keine signifikante Verbesserung gegenüber einzelnen KI-Aufrufen erreichen. Zwar konnten in einzelnen Bereichen Verbesserungen gemessen werden, gleichzeitig traten jedoch auch Verschlechterungen auf. Insgesamt gleichen sich positive und negative Effekte weitgehend aus, sodass sich kein klarer Vorteil der Workflows nachweisen lässt.

Eine weiterführende Analyse zeigt jedoch, dass Workflows schwächere Antworten teilweise verbessern können. Gleichzeitig verschlechtern sie bereits gute Antworten relativ häufig. Dadurch entsteht im Gesamtergebnis keine messbare Qualitätssteigerung. Dieses Muster zeigte sich sowohl in den Hauptversuchen als auch im zusätzlichen Kontrollversuch. Besonders bemerkenswert ist, dass die Resultate den ursprünglichen Erwartungen widersprachen. Statt einer deutlichen Verbesserung durch zusätzliche Verarbeitungsschritte ergaben die Untersuchungen Hinweise auf grundlegende Grenzen bei der Selbstkorrektur von KI-Systemen.

Die Arbeit legt nahe, dass der Aufbau leistungsfähiger KI-Workflows anspruchsvoller ist als häufig angenommen wird. Zusätzliche Verarbeitungsschritte führen nicht automatisch zu besseren Ergebnissen und können neue Fehlerquellen schaffen. Die Untersuchung zeigt damit Grenzen aktueller Ansätze zur automatischen Selbstkorrektur von KI-Systemen auf und liefert wertvolle Hinweise für deren praktischen Einsatz in Softwarelösungen.
...
# 5. Fazit
Anfangs war die Forschungsfrage ob Workflows qualitativ bessere Antworten geben als Einzelaufrufe. Die Tests konnten dies nicht ziegen. Somit haben sich ganz neue Forschungsfragen ergeben: Wie verbessere ich meine Messung. Kann ich einen KI Schritt als Arbeitsschritt verwenden, wenn er so viele Fehler erzeugt wie in dieser Stichprobe und könnte es sein, dass eine KI die Antwort der KI gar nicht verbessern kann? 

. 

# 5.1  Ergebnisse der Arbeit:

Sind Workflows besser als einzelne Aufrufe? In der Stichprobe konnte gesamthaft und auf allen wichtigen Metriken kein signifikanter Unterschied in positiver Richtung, also keine signifikante Verbesserung, gemessen werden.
Die Ergebnisse konnten die Nullhypothese nicht verwerfen. Der zweiseitige gepaarte t-Test zeigte keine signifikante Veränderung, weder in positiver noch in negativer Richtung. Die Nullhypothese besagt, dass die Antworten von Workflows und Einzelaufrufen die gleiche Qualität haben.

Dies ist besonders unerwartet, da KI bekannte Fähigkeiten hat und diese eigentlich einfach testbar sein sollten.

Daneben wurden mögliche Ursachen und deren Auswirkungen erörtert:


Kann ich die Tests verbessern? Ich habe viele verschiedene Tests durchgeführt, um Verzerrungen zu verringern und sogar erwartete Ergebnisse gezielt sichtbar zu machen. Trotzdem konnte mit dieser Stichprobengrösse kein klares Signal oberhalb des Rauschens gefunden werden. 

Kann ich KI als Arbeitsschritt in einer Software sinnvoll verwenden? Die Verschlechterungen, die die KI in dieser Stichprobe zeigt, sind bereits ein deutlich Hinweis darauf, dass gewisse KI-Schritte in Software nicht nur nutzlos sind, sondern auch schaden können. Ich kam zur Erkenntnis, dass Workflows schwieriger zu bauen sind, als sie nur nach sinnvollen Arbeitsschritten zusammenzusetzen. Es scheint eine technische Limitierung zu geben. Es würde sich lohnen, dies gezielt zu untersuchen.

Kann eine KI die Antwort derselben KI verbessern, ohne zu verschlechtern, also zwischen richtig und falsch unterscheiden, wenn der Input von einer KI kommt? Es werden verschiedene Erklärungsrichtungen erörtert, auch die Möglichkeit, dass die KI mir unbekannte Eigenschaften hat. Ich analysiere den Kern der Frage: Was genau müssten Workflows können, damit die gemessenen Verschlechterungen nicht eintreten? Meine Messungen in dieser Stichprobe lassen vermuten, dass es eine technische Limitierung gibt. Es würde sich lohnen, dies gezielt zu untersuchen.
...
Am wenigsten traue ich meinen von KI generierten Fragen.
