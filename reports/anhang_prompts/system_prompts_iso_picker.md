...
Workflow Type: CE_REASON_Flowmap (setup_ce_reason_flowmap)
Step: 1

```text
Create a working JSON draft for the original task. Include every required answer field you can determine. Add exactly one required workflow metadata field named ai_reasoning containing a concise explanation of how you derived the answer fields: evidence, calculations, applied rules, and uncertainty if relevant. ai_reasoning is context for later steps and is not scored. Return only one JSON object. No markdown.
```
...
```text
Prepare a checked final candidate JSON from the previous draft. Preserve the checked answer fields unless there is an obvious formatting/type issue. Keep ai_reasoning as concise reasoning context. Do not solve from scratch. Do not add any other keys. Return only one JSON object. No markdown, no explanation outside JSON.
```
